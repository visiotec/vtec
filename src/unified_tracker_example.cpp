...
int main(int argc, char** argv)
{
  ...
  VTEC::UnifiedHomographyOptimizer optimizer;
  ...
  for (i = 0; i < 10; ++i)
  {
    ...
    cv::Mat current_image = cv::imread(fileNameStream.str(), cv::IMREAD_GRAYSCALE); //old version CV_LOAD_IMAGE_GRAYSCALE);CV_LOAD_IMAGE_GRAYSCALE);
